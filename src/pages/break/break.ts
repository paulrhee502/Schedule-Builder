...
import { AddBreakModalPage } from '../add-break-modal/add-break-modal';
...
export class BreakPage {
  ...
  public loaded = false;
  constructor(public navCtrl: NavController, public navParams: NavParams, public viewCtrl: ViewController, public http: Http, public alertCtrl: AlertController, public modalCtrl: ModalController, public storage: Storage) {
  ...
  addBreak(){
    let addBreakModal = this.modalCtrl.create(AddBreakModalPage);
    addBreakModal.onDidDismiss(data => {
      if(data != null){
        this.viewCtrl.dismiss();
      }
    })
    addBreakModal.present();
  }
}
